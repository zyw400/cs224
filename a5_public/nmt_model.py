...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # print("target_padded: {}".format(target_padded.size()))
        # print("index size: {}".format(target_padded[1:].unsqueeze(-1).size()))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Y = F.embedding(target_padded, self.model_embeddings_target.weight)
        # print("Y {}".format(Y.size()))

        # i = 0
        for y1 in torch.split(Y, split_size_or_sections = 1):
            Y_t = torch.squeeze(y1, dim=0)
        ...
        # o_prev = torch.zeros(batch_size, self.hidden_size, device=self.device)    
        # for Y_t2 in Y:
        #     Ybar_t2 = torch.cat([Y_t2, o_prev], -1)
            

        #     dec_state, o_t, _ = self.step(Ybar_t2, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
        #     combined_outputs.append(o_t)
        #     o_prev = o_t
        #     if i == 0:
        #         print("Ybar_t2 {}".format(o_t))
        #     i += 1
        # print(s == s2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print("enc_hiddens size: {}".format(enc_hiddens.size()))
        ### END YOUR CODE FROM ASSIGNMENT 4
        ...
        # print("alpha_t size: {}".format(alpha_t.size()))
