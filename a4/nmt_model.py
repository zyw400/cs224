...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, hidden_size, num_layers = 1, bias = True, bidirectional = True, dropout=self.dropout_rate,)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens_unpacked, source_lengths2 = nn.utils.rnn.pad_packed_sequence(enc_hiddens_raw, ) # default padding value is 0.0!

        # print(enc_hiddens_unpacked)
        enc_hiddens = enc_hiddens_unpacked.permute(1, 0, 2)
       
        last_hidden_conc = torch.cat((last_hidden[0],last_hidden[1]), 1) # assuming that the last h_1<- is the first element!
        init_decoder_hidden = self.h_projection(last_hidden_conc)

        last_cell_conc = torch.cat((last_cell[0],last_cell[1]), 1) # assuming that the last h_1<- is the first element!
        init_decoder_cell = self.c_projection(last_cell_conc)
